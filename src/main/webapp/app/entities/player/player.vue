<template>
  <div>
    <h2 id="page-heading" data-cy="PlayerHeading">
      <span id="player-heading">Players</span>
      ...
      <table class="table table-striped" aria-describedby="players">
        ...
        <tbody>
          <tr v-for="player in players" :key="player.id" data-cy="entityTable">
            ...
            <td>{{ player.plyrNm }}</td>
            <td>{{ player.plyrLvl }}</td>
            <td>{{ player.lstMtnUsr }}</td>
            <td>{{ player.lstMtnDt | formatDate }}</td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="pbl230801App.player.delete.question" data-cy="playerDeleteDialogHeading">Confirm delete operation</span></span
      >
      <div class="modal-body">
        <p id="jhi-delete-player-heading">Are you sure you want to delete this Player?</p>
      </div>
